...
if (!host) {
  throw new Error("#hero-canvas-host が見つかりません。");
}
...
const clusterGlow = createGroundGlow(7.6, 5.2, 0xc7e8ff, 0.18);
clusterGlow.position.set(1.35, -1.24, 0.2);
scene.add(clusterGlow);

const frontGlow = createGroundGlow(4.0, 2.5, 0x8fe0ff, 0.12);
frontGlow.position.set(0.35, -1.2, 1.55);
scene.add(frontGlow);

const trailGlow = createGroundGlow(7.2, 2.2, 0x89d8ff, 0.085);
trailGlow.position.set(0.35, -1.24, -0.4);
scene.add(trailGlow);

const floorVeil = createGroundGlow(10.5, 3.2, 0xa7deff, 0.05);
floorVeil.position.set(1.1, -1.24, 0.45);
scene.add(floorVeil);
...
const materialThemeState = {
  cyan: 1,
  navy: 0,
  ice: 0,
};

const materialThemeTarget = {
  cyan: 1,
  navy: 0,
  ice: 0,
};
...
function mixColor(target, baseHex, emphasisHex, weight) {
  target.color
    .setHex(baseHex)
    .lerp(new THREE.Color(emphasisHex), weight);
}
...
function applyMaterialTheme() {
  ...
  mixColor(materials.cyan, 0x68e8ff, 0x21f0ff, cyanWeight * 0.86 + iceWeight * 0.16);
  mixEmissive(materials.cyan, 0x2fdfff, 0x00e6ff, cyanWeight * 0.95);
  materials.cyan.emissiveIntensity = 0.38 + cyanWeight * 0.42 + pulseWeight * 0.1;
  materials.cyan.opacity = 0.66 + cyanWeight * 0.06;
  materials.cyan.envMapIntensity = 1.95 + cyanWeight * 0.38 + pulseWeight * 0.1;
  ...
  materials.navy.emissiveIntensity = 0.24 + navyWeight * 0.2 + pulseWeight * 0.035;
  materials.navy.opacity = 0.78 + navyWeight * 0.08;
  materials.navy.envMapIntensity = 1.72 + navyWeight * 0.32 + pulseWeight * 0.08;
  materials.navy.thickness = 3.05 + navyWeight * 0.34;

  mixColor(materials.ice, 0xe8fbff, 0xffffff, iceWeight * 0.75 + cyanWeight * 0.15);
  mixEmissive(materials.ice, 0xc5f5ff, 0xe6fdff, iceWeight * 0.8);
  materials.ice.opacity = 0.46 - iceWeight * 0.08 + cyanWeight * 0.03;
  materials.ice.emissiveIntensity = 0.09 + iceWeight * 0.2 + cyanWeight * 0.05 + pulseWeight * 0.045;
  ...
  materials.clear.emissiveIntensity = 0.05 + iceWeight * 0.14 + pulseWeight * 0.035;
  materials.clear.envMapIntensity = 1.95 + iceWeight * 0.42 + pulseWeight * 0.08;

  mixColor(materials.paleBlue, 0xa7d9ff, 0x62dfff, cyanWeight * 0.62 + iceWeight * 0.18);
  ...
  materials.paleBlue.envMapIntensity = 1.72 + iceWeight * 0.18 + cyanWeight * 0.18;

  mixColor(materials.lime, 0xcfff79, 0xdfff55, iceWeight * 0.16 + cyanWeight * 0.08);
  mixEmissive(materials.lime, 0xbaff62, 0xd6ff60, iceWeight * 0.24 + pulseWeight * 0.1);
  materials.lime.emissiveIntensity = 0.2 + cyanWeight * 0.04 + iceWeight * 0.035 + pulseWeight * 0.035;
  materials.lime.envMapIntensity = 1.62 + iceWeight * 0.12;

  mixColor(materials.mint, 0x6af4cf, 0x34f2d5, cyanWeight * 0.72 + iceWeight * 0.18);
  ...
  mixColor(materials.amber, 0xe9f7d4, 0xf7fff0, iceWeight * 0.36 + cyanWeight * 0.08);
  mixEmissive(materials.amber, 0xd6ffe1, 0xf2fff9, iceWeight * 0.45 + pulseWeight * 0.1);
  ...
  materials.amber.envMapIntensity = 1.64 + iceWeight * 0.14 + pulseWeight * 0.035;

  jewelCoreMaterials.cyan.opacity = 0.22 + cyanWeight * 0.11 + pulseWeight * 0.08;
  jewelCoreMaterials.paleBlue.opacity =
    0.14 + cyanWeight * 0.08 + iceWeight * 0.05 + pulseWeight * 0.04;
  jewelCoreMaterials.navy.opacity = 0.16 + navyWeight * 0.1 + pulseWeight * 0.035;
  jewelCoreMaterials.lime.opacity = 0.18 + iceWeight * 0.05 + pulseWeight * 0.035;
  jewelCoreMaterials.mint.opacity = 0.2 + cyanWeight * 0.12 + pulseWeight * 0.07;
  jewelCoreMaterials.amber.opacity = 0.08 + iceWeight * 0.045 + pulseWeight * 0.02;

  jewelEdgeMaterials.cyan.opacity = 0.34 + cyanWeight * 0.1 + pulseWeight * 0.07;
  jewelEdgeMaterials.paleBlue.opacity =
    0.26 + cyanWeight * 0.06 + iceWeight * 0.04 + pulseWeight * 0.04;
  jewelEdgeMaterials.navy.opacity = 0.26 + navyWeight * 0.09 + pulseWeight * 0.04;
  jewelEdgeMaterials.lime.opacity = 0.3 + iceWeight * 0.04 + pulseWeight * 0.035;
  jewelEdgeMaterials.mint.opacity = 0.32 + cyanWeight * 0.09 + pulseWeight * 0.06;
  jewelEdgeMaterials.amber.opacity = 0.18 + iceWeight * 0.04 + pulseWeight * 0.02;
  jewelEdgeMaterials.ice.opacity = 0.24 + iceWeight * 0.08 + pulseWeight * 0.035;
}

const cluster = new THREE.Group();
cluster.position.set(1.42, 0.18, -0.2);
cluster.rotation.y = -0.38;
cluster.rotation.x = 0.06;
cluster.scale.setScalar(0.9);
scene.add(cluster);
...
function resize() {
  ...
  if (width < 900) {
    camera.position.set(0.6, 1.95, 13.7);
    baseClusterTransform.positionX = 1.08;
    baseClusterTransform.positionY = -0.02;
    baseClusterTransform.positionZ = -0.1;
    baseClusterTransform.scale = 0.8;
  } else {
    camera.position.set(0.8, 2.0, 12.5);
    baseClusterTransform.positionX = 1.42;
    baseClusterTransform.positionY = 0.18;
    baseClusterTransform.positionZ = -0.2;
    baseClusterTransform.scale = 0.9;
  }
...
function animate() {
  ...
  if (!prefersReducedMotion.matches) {
    ...
    frontGlow.material.opacity =
      0.11 + Math.sin(elapsed * 1.1) * 0.015 + themePulse * 0.025;
    clusterGlow.material.opacity =
      0.17 + Math.sin(elapsed * 0.8) * 0.02 + themePulse * 0.018;
    trailGlow.material.opacity =
      0.08 + Math.sin(elapsed * 0.7) * 0.012 + themePulse * 0.012;
    floorVeil.material.opacity = 0.05 + Math.sin(elapsed * 0.55) * 0.008;
  }
