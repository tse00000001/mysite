...
const lyricsWords = ['NIGHT', 'SYSTEM', 'HACK', 'PULSE', 'URBAN', 'RUSH', 'NEON', 'GLITCH'];
...
function animate() {
    ...
    if (isPlaying && analyser) {
        ...
        if (bassAvg > 220 && Date.now() - lastBeatTime > 250) {
            lyricFlash.textContent = lyricsWords[Math.floor(Math.random() * lyricsWords.length)];
            lyricFlash.style.opacity = 1;
            camera.position.x = (Math.random() - 0.5) * 0.5;
            camera.position.y = (Math.random() - 0.5) * 0.5;
            lastBeatTime = Date.now();
        } else {
            lyricFlash.style.opacity = Math.max(0, Number(lyricFlash.style.opacity) - 0.08);
